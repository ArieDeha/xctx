...
function mkCodec(opts) {
    ...
    return new Codec(cfg);
}
function withDateNow(fakeMs, fn) {
    const real = Date.now;
    Date.now = () => fakeMs;
    try {
        return fn();
...
describe('codec', () => {
    ...
    it('roundtrip without AAD binder (null)', () => {
        const c = mkCodec({ aadBinder: null }); // explicit null path
    ...
    it('rejects tamper (ciphertext changed -> auth failed)', () => {
        ...
        const env = b64url.decodeJson(val.slice(3));
    ...
    it('rejects payload json parse failed (cipher ok, pt junk)', () => {
        ...
        const badEnv = b64url.decodeJson(badVal.slice(3));
        const env2 = { ...env, ct: badEnv.ct }; // wrong ciphertext/tag for our key
