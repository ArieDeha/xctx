...
describe('express middleware', () => {
    it('400 when header missing', () => {
        ...
        xctxMiddleware(codec)(req, res, () => { nextCalled = true; });
    ...
    it('401 when parse fails', () => {
        const codec = mkCodec();
        const req = { header: () => 'v1.bad' }; // malformed
        const res = mkRes();
        xctxMiddleware(codec)(req, res, () => { });
    ...
    it('success attaches payload and claims', () => {
        ...
        const req = { header: (h) => (h === name ? val : null) };
        const res = mkRes();
        let nextCalled = false;
        xctxMiddleware(codec)(req, res, () => { nextCalled = true; });
        expect(nextCalled).toBe(true);
        expect(req.xctxPayload.user_name).toBe('arie');
        expect(req.xctxClaims.iss).toBe('svc-caller');
    });
});
